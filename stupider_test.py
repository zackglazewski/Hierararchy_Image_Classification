...
def get_data_loaders(data_dir, batch_size, train = False):
    if train:
        ...
        train_data_len = len(all_data)
        # valid_data_len = int((len(all_data) - train_data_len)/2)
        # test_data_len = int(len(all_data) - train_data_len - valid_data_len)
        # train_data, val_data, test_data = random_split(all_data, [train_data_len, valid_data_len, test_data_len])
        train_loader = DataLoader(all_data, batch_size=batch_size, shuffle=True, num_workers=0)
...
data_dir = "inat2021birds/bird_train_mini"
(train_loader, train_size) = get_data_loaders(data_dir, batch_size=2, train=True)
